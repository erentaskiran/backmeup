...
remove(){
    ...
    local updated_crontab=$(echo "$current_crontab" | grep -v "$(basename "$script_path")")
...
update(){
    ...
    if [[ -z "$script_path" || -z "$time_period" ]]; then
        log_error "Script path and time period are required"
        return 1
    fi
